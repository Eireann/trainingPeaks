...
alignment with Mark Millerâ€™s TC39 [strawman][] for concurrency. At the same
...
-   `delete` and `set` (nÃ©e `put`) no longer have a fulfillment value.
...
## 0.8.5
 ...
 - Support Internet Explorers 7â€“9 (with multiple bug fixes @domenic)
 - Support Firefox 12
...
## 0.8.4
 ...
 - Added experimental ``Q.promise(maker(resolve, reject))`` to make a
   ...
   @gozalaâ€™s stream constructor pattern and the Microsoft Windows Metro
