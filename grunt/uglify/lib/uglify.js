...
exports.init = function(grunt) {
  ...
  exports.minify = function(files, dest, options) {
    ...
    files.forEach(function(file){
      ...
      // TP Changes: store original code in source map.
      if(outputOptions.source_map) {
        outputOptions.source_map.get().setSourceContent(file, code);
      }
    });

    // TP Changes: Expand sourceURL comments
    topLevel = (function expand(toplevel, source_map) {
      function tryExpand(node) {
        ...
        if (node instanceof UglifyJS.AST_Call &&
          ...
          node = UglifyJS.parse(code, { filename: path });
          node = new UglifyJS.AST_BlockStatement(node);
        ...
        var expandSourceURL = new UglifyJS.TreeTransformer(function(node, descend) {
            if(node instanceof UglifyJS.AST_SimpleStatement) {
              return tryExpand(node.body);
            } else {
              return tryExpand(node);
            }
        });

        return toplevel.transform(expandSourceURL);
    })(topLevel, outputOptions.source_map);
